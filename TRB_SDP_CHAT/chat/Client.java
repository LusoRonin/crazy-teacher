...
public class Client extends Frame {
    ...
    public boolean action(Event i, Object o) {
        if (i.target == Send) {
            ...
            int iter = 0;
            String destArray [] = addr.getText().split(",");
            
            for (int j = 0; j < tmpPortDest.length(); j = j + 4){
                String strPortDest = tmpPortDest.substring(j, j + 4);
                System.out.print("\niter: " + iter + "\nj: " + j + "\nstrPortDest: " + strPortDest);
                if(strPortDest.equals("1234")){
                    ecran.append("\n" + "-Didn't send to user '" + destArray[iter] + "' because it was not found!");
                }else{        
